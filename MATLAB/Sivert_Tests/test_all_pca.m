    ...
    for temp = 1:size(ids,2)
        for a = 1:length(td_algs)
            ...
            if td_alg == "hyperOsp"
                ...
                B = getBackground(M_new_pca, numEndmembers, end_sign_new);
                [a1, b1, ~] = td_results(M_new_pca, gt, end_name, end_sign_new,...
            ...
            result = sprintf('%s;%d;%d;%d\n',name,max_score,p_c_a,p_f_a);
...
fid = fopen('result_full_pca20acer_rt_rtsm.csv','a');
