function [res_img] = tdRun(M, end_sign, td_alg, train_set, background)
...
% train_set,  the samples used for training
...
if and(isstring(background), isstring(train_set))
    res_2d = feval(td_alg,M2d, end_sign);
elseif not(isstring(background))
   res_2d = feval(td_alg, M2d, background, end_sign);
else
    res_2d = feval(td_alg, M2d, end_sign, train_set);
end
