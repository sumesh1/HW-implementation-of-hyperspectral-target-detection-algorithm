...
for i = 1: num_pixels
    ...
    if(sw==0)
        ...
        SRs   = fi(targ_fp' * TRs,1, wlen, 20 + (wlen-35), fm);
        SRx   = fi(targ_fp' * TDTRx, 1, wlen, 20 + (wlen-35), fm);
        XRx   = fi(u' * TDTRx, 1, wlen, 20 + (wlen-35), fm);%33
        SRx2  = fi(SRx*SRx, 1, wlen, 20 + (wlen-35), fm);
        XRx2   = fi(XRx*XRx, 1, wlen, 20 + (wlen-35), fm); %34
        tmp2  = fi(SRs*XRx, 1, wlen, 20 + (wlen-35), fm); %27
        SRxAb = fi(abs(SRx), 1, wlen, 20 + (wlen-35), fm);
        XRxAb = fi(abs(XRx), 1, wlen, 20 + (wlen-35), fm); %33
        tmp3  = fi(SRx*SRxAb, 1, wlen, 20 + (wlen-35), fm);
        tmp4  = fi(SRs*XRxAb, 1, wlen, 20 + (wlen-35), fm); %overflow -27
        tmp5  = fi(SRx*SRx2, 1, wlen, 20 + (wlen-35), fm); %overflow 28
        tmp6  = fi(SRs*XRx2, 1, wlen, 20 + (wlen-35), fm); %overflow -27
    else
...
%         tmp6  = fi(SRs*XRx2, 1, wlen, 26 + (wlen-35), fm); %overflow -27
        TDTRx = fi(A * u, 1, wlen, 24 + (wlen-35), fm);
        TRs   = fi(A * targ_fp , 1 , wlen, 24 + (wlen-35), fm);
        SRs   = fi(targ_fp' * TRs,1, wlen, 24 + (wlen-35), fm);
        SRx   = fi(targ_fp' * TDTRx, 1, wlen, 24 + (wlen-35), fm);
        XRx   = fi(u' * TDTRx, 1, wlen, 24 + (wlen-35), fm);%33
        SRx2  = fi(SRx*SRx, 1, wlen, 24 + (wlen-35), fm);
        XRx2   = fi(XRx*XRx, 1, wlen, 24 + (wlen-35), fm); %34
        tmp2  = fi(SRs*XRx, 1, wlen, 24 + (wlen-35), fm); %27
        SRxAb = fi(abs(SRx), 1, wlen, 24 + (wlen-35), fm);
        XRxAb = fi(abs(XRx), 1, wlen, 24 + (wlen-35), fm); %33
        tmp3  = fi(SRx*SRxAb, 1, wlen, 24 + (wlen-35), fm);
        tmp4  = fi(SRs*XRxAb, 1, wlen, 24 + (wlen-35), fm); %overflow -27
        tmp5  = fi(SRx*SRx2, 1, wlen, 24 + (wlen-35), fm); %overflow 28
        tmp6  = fi(SRs*XRx2, 1, wlen, 24 + (wlen-35), fm); %overflow -27
    end
...
%     tmp6  = fi(SRs*XRx2, 1, wlen, 15   + (wlen-35), fm); %overflow -27
    ...
    if (sw==0)
        ...
        TRxxR = fi(TRx * TRx', 1, wlen + 7, 20 + 7 + (wlen-35), fm);
        Td = fi(u' * TRx, 1, wlen, 21 + (wlen-35), fm);%15, fm);
        Tdiv = fi(1/(double(Td)+1), 1, wlen, 32 + (wlen-35), fm);
        TT = fi(TRxxR .* Tdiv, 1, wlen, 24 + (wlen-35), fm);
        Anew = fi(A - TT, 1, wlen, 24  + (wlen-35), fm); 
    end
