...
static s32 R32[N_bands][N_bands]={};
...
static s32 sR32[N_bands]={};
static double xR[N_bands]={};
//static double result[N_pixels]={};
static  datatype target[N_bands]={};
static u32 *BRAM_BASE_ADDR = XPAR_BRAM_WRAPPER_0_BASEADDR;
static s32 receiver [N_pixels]={};
/*****************************************************************************/
...
void hyperCorr (datatype *m, double (*R)[N_bands], int NB, int NP, int percent)
{
	...
	for(p=0; p<NP; p=p+jump)
	{
		...
		for (i=0; i<NB; i++)
		{
			for (j=0; j<NB; j++)
			{
				R[i][j]=R[i][j]+((double)m[k+i]*m[k+j]/(double)divisor);
			}
		}
	}
}
