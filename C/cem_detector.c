...
//FUNCTIONS
double scalarProduct (double*, s16*, int);
int FfsSd(char*,s16 * );
...
void hyperCorr (s16 *, double (*)[], int, int,int);
...
void arrayMatrixProduct(int , s16 *, double [][N_bands], double *);
...
static char ResultsFile[32] = "cem.bin";
...
double scalarProduct (double* a, s16* b, int N)
{
    int i;
    double sum=0;
        for (i=0;i<N;i++)
...
double SAM (s16 *x, s16 *s, u64 target_product, int N)
{
...
void hyperCorr (s16 *m, double (*R)[N_bands], int NB, int NP, int percent)
{
	int i,j,p,k=0;
...
void arrayMatrixProduct(int size, s16 *A, double R[][N_bands], double *result)
{
	int i,j;
	double sum;
	for (i=0; i<size; i++)
...
void removeMean(s16* M,s32* R,int NB, int NP)
{
	s32 mean[N_bands]={};
	int i,j,k=0;
	for (j=0; j<NB;j++)
